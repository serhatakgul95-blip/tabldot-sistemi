...
function firebaseErrorMessage(error) {
  ...
  const map = {
    'auth/email-already-in-use':
      'Bu telefon numarasıyla daha önce kayıt oluşturulmuş.',
    ...
    'auth/operation-not-allowed':
      'Firebase Authentication içinde Email/Password giriş yöntemi henüz etkinleştirilmemiş.'
  };
...
async function bootstrapAdmin({
  ...
  if (await hasAnyAdmin()) {
    throw new Error(
      'Sistemde zaten bir admin hesabı bulunuyor.'
    );
  }
  ...
  const profile =
    await createProfile(
      cred.user.uid,
      {
        ...
        annualAllowance:
          existing?.annualAllowance ?? 30,

        usedLeave:
          existing?.usedLeave ?? 0,

        planningScore:
          existing?.planningScore ?? 50,

        planningScoreNote:
          existing?.planningScoreNote || ''
      }
    );
...
function stateToMaps(
  ...
  maps.leavePreferences =
    arrayMap(
      state.leavePreferences,
      x =>
        `${x.userId}_${x.year}`
    );


  maps.leavePlanResults =
    arrayMap(
      state.leavePlanResults,
      x =>
        `${x.userId}_${x.year}`
    );
...
async function loadState(
  ...
  const [
    ...
    await Promise.all([
      ...
      collectionData(
        COLLECTIONS.leavePreferences
      ),

      collectionData(
        COLLECTIONS.leavePlanResults
      ),
  ...
  meals.forEach(
    ({
      ...
      state.mealSelections[userId][date] =
        {
          breakfast,
          lunch,
          dinner
